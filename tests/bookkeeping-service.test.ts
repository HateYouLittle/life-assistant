...
test("account CRUD with initial balance and derived balance", () => {
  const a = requireProfileContext("profile-a");
  const account = svc.createAccount(a, { name: "现金钱包", type: "cash", initialBalance: 100.5 });
  ...
  const unused = svc.createAccount(a, { name: "临时账户" });
...
test("entries adjust derived balances including double-sided transfers", async () => {
  const a = requireProfileContext("profile-a");
  const wallet = svc.createAccount(a, { name: "余额测试-钱包", type: "cash" });
  const bank = svc.createAccount(a, { name: "余额测试-银行卡", type: "bank", initialBalance: 200 });
...
test("category rules: required for expense/income, forbidden for transfer", async () => {
  const a = requireProfileContext("profile-a");
  const wallet = svc.createAccount(a, { name: "分类规则-钱包" });
  ...
  // transfer 需要两个不同账户
  const other = svc.createAccount(a, { name: "分类规则-银行卡" });
  await assert.rejects(
...
test("summary aggregates income/expense/categories by calendar month in local timezone", async () => {
  ...
  const wallet = svc.createAccount(s, { name: "汇总-钱包" });
